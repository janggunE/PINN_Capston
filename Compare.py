...
model.restore("0424-30000.pt")  # Replace ? with the exact filename


#load CFD Data
CFD_Data=pd.read_csv("CFD.csv")
df=CFD_Data.drop([0,1,2,3],axis=0)
df.columns=['x','y','u','v','p']

x=df["x"].values.astype(np.float64)-0.5
y=df["y"].values.astype(np.float64)
u=df["u"].values.astype(np.float64).reshape(-1,1)
v=df["v"].values.astype(np.float64).reshape(-1,1)
p=df["p"].values.astype(np.float64).reshape(-1,1)
...
relative_Err=result_PINN-result_CFD
#print(result[:,2])
print(array.shape)
